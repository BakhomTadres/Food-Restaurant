...
export default function Checkout() {
  ...
  const handlePaymentChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setPaymentForm({ ...paymentForm, [name]: type === "checkbox" ? checked : formatted });
  ...
  const validateDelivery = () => {
    ...
    else if (delivery_form.phone.replace(/\D/g, "").length < 10) e.phone = "Enter a valid phone number";
    if (!delivery_form.email.trim()) e.email = "Required";
    else if (!delivery_form.email.includes("@")) e.email = "Enter a valid email";
  ...
  const validatePayment = () => {
    ...
    if (paymentForm.method === "card") {
      ...
      if (!paymentForm.expiry || paymentForm.expiry.length < 5) e.expiry = "Required";
  ...
  if (cartItems.length === 0 && !submitted) {
    return (
      <div className="min-h-screen bg-gray-900 flex flex-col items-center justify-center gap-6 px-4">
        <Header location="menu" elements={elements} setElements={setElements} setElementsNum={setElementsNum} elementsnum={elementsNum} />
        <div className="text-center mt-20">
          <div className="w-24 h-24 bg-gray-800 rounded-full flex items-center justify-center mx-auto mb-6">
            <i className="fa-solid fa-cart-shopping text-4xl text-gray-600"></i>
          </div>
          <h2 className="text-2xl font-black text-amber-50 mb-3">Your cart is empty</h2>
          <p className="text-gray-400 mb-8">Add some dishes before checking out.</p>
          <button
  ...
  if (submitted) {
    return (
      <div className="min-h-screen py-4 bg-gray-900 flex flex-col items-center justify-center px-4">
        <Header location="menu" elements={[]} setElements={setElements} setElementsNum={setElementsNum} elementsnum={0} />
        <div className="max-w-md w-full text-center mt-20">
          ...
          <h1 className="text-4xl font-black text-amber-50 mb-3">Order Placed!</h1>
          ...
          <div className="bg-gray-800 border border-white/5 rounded-2xl p-6 mb-8 text-left space-y-3">
            <div className="flex justify-between text-sm">
              ...
              <span className="text-amber-50 font-semibold">{delivery_form.firstName} {delivery_form.lastName}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-400">Estimated time</span>
              <span className="text-amber-50 font-semibold">30 – 45 minutes</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-400">Total paid</span>
              <span className="text-amber-400 font-black text-base">${total.toFixed(2)}</span>
            </div>
          </div>
            ...
            {[
              ...
              <div key={label} className="flex items-center">
                <div className="flex flex-col items-center gap-1">
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm ${done ? "bg-amber-400 text-gray-900" : "bg-gray-700 text-gray-500"}`}>
                    <i className={`fa-solid ${icon}`}></i>
                  </div>
                  <span className={`text-xs font-semibold ${done ? "text-amber-400" : "text-gray-600"}`}>{label}</span>
                </div>
                {i < arr.length - 1 && (
                  <div className={`h-0.5 w-8 mx-1 mb-4 ${done ? "bg-amber-400" : "bg-gray-700"}`}></div>
                )}
              </div>
            ))}
          </div>
  ...
  return (
    <div className="min-h-screen bg-gray-900">
      ...
      <div className="max-w-7xl mx-auto px-4 pt-28 pb-20">
        ...
        {/* Page Title */}
        <div className="mb-10">
          <button onClick={() => navigate("/menu")} className="flex items-center gap-2 text-gray-400 hover:text-amber-400 transition text-sm cursor-pointer mb-4">
                ...
                <div className="flex flex-col items-center gap-1.5">
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-300 ${
                      ...
                      isActive ? "bg-amber-400 text-gray-900" :
                      "bg-gray-800 border border-white/10 text-gray-500"
                    }`}
                  >
                    {isDone ? <i className="fa-solid fa-check text-xs"></i> : <i className={`fa-solid ${icon} text-xs`}></i>}
                  </div>
                  <span className={`text-xs font-semibold ${isActive ? "text-amber-400" : isDone ? "text-green-400" : "text-gray-500"}`}>
                    {label}
                  </span>
                </div>
                {i < STEPS.length - 1 && (
                  <div className={`h-0.5 flex-1 mx-3 mb-5 transition-all duration-500 ${isDone ? "bg-green-500" : "bg-gray-700"}`}></div>
                )}
              </div>
            );
          })}
              ...
              <div className="bg-gray-800 border border-white/5 rounded-3xl p-6 md:p-8">
                ...
                <div className="flex gap-3 mb-6">
                  {(["delivery", "pickup"] as const).map((m) => (
                    <button
                      key={m}
                      onClick={() => setDeliveryForm({ ...delivery_form, method: m })}
                    ...
                    >
                      <i className={`fa-solid ${m === "delivery" ? "fa-motorcycle" : "fa-store"}`}></i>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    {[
                      ...
                      { name: "lastName", label: "Last Name", placeholder: "Mohamed" },
                    ].map(({ name, label, placeholder }) => (
                      <div key={name}>
                        <label className="block text-xs font-semibold text-gray-400 mb-1.5 uppercase tracking-wide">{label} <span className="text-amber-400">*</span></label>
                        <input
                          name={name}
                        ...
                        />
                        {deliveryErrors[name] && <p className="text-red-400 text-xs mt-1">{deliveryErrors[name]}</p>}
                      </div>
                    ))}
                  </div>
                  ...
                  {delivery_form.method === "delivery" && (
                    <>
                      <div>
                        <label className="block text-xs font-semibold text-gray-400 mb-1.5 uppercase tracking-wide">Street Address <span className="text-amber-400">*</span></label>
                        <input
                          name="address"
                        ...
                        />
                        {deliveryErrors.address && <p className="text-red-400 text-xs mt-1">{deliveryErrors.address}</p>}
                      </div>

                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-xs font-semibold text-gray-400 mb-1.5 uppercase tracking-wide">Apartment / Floor</label>
                          <input
                            name="apartment"
                        ...
                        <div>
                          <label className="block text-xs font-semibold text-gray-400 mb-1.5 uppercase tracking-wide">City <span className="text-amber-400">*</span></label>
                          <input
                            name="city"
                          ...
                          />
                          {deliveryErrors.city && <p className="text-red-400 text-xs mt-1">{deliveryErrors.city}</p>}
                        </div>
                      </div>
                    </>
                  )}
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-gray-400 mb-1.5 uppercase tracking-wide">Order Notes</label>
                    <textarea
                      name="notes"
            ...
            {currentStep === "payment" && (
              <div className="bg-gray-800 border border-white/5 rounded-3xl p-6 md:p-8">
                ...
                <div className="grid grid-cols-3 gap-3 mb-6">
                  {[
                    { key: "card", icon: "fa-credit-card", label: "Credit Card" },
                    { key: "cash", icon: "fa-money-bill-wave", label: "Cash on Delivery" },
                    { key: "wallet", icon: "fa-wallet", label: "Digital Wallet" },
                  ].map(({ key, icon, label }) => (
                    <button
                      key={key}
                      onClick={() => setPaymentForm({ ...paymentForm, method: key as any })}
                    ...
                    <div className="relative bg-linear-to-br from-gray-700 to-gray-900 border border-white/10 rounded-2xl p-5 h-40 overflow-hidden">
                      ...
                      <div className="relative">
                        ...
                        <div className="flex justify-between">
                          <p className="text-white/50 text-xs">{paymentForm.cardName || "CARDHOLDER NAME"}</p>
                          <p className="text-white/50 text-xs">{paymentForm.expiry || "MM/YY"}</p>
                        </div>
                      </div>
                    </div>

                    <div>
                      <label className="block text-xs font-semibold text-gray-400 mb-1.5 uppercase tracking-wide">Cardholder Name <span className="text-amber-400">*</span></label>
                      <input
                        name="cardName"
                      ...
                      />
                      {paymentErrors.cardName && <p className="text-red-400 text-xs mt-1">{paymentErrors.cardName}</p>}
                    </div>

                    <div>
                      <label className="block text-xs font-semibold text-gray-400 mb-1.5 uppercase tracking-wide">Card Number <span className="text-amber-400">*</span></label>
                      <div className="relative">
                        <input
                          name="cardNumber"
                      ...
                      {paymentErrors.cardNumber && <p className="text-red-400 text-xs mt-1">{paymentErrors.cardNumber}</p>}
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-semibold text-gray-400 mb-1.5 uppercase tracking-wide">Expiry Date <span className="text-amber-400">*</span></label>
                        <input
                          name="expiry"
                        ...
                        />
                        {paymentErrors.expiry && <p className="text-red-400 text-xs mt-1">{paymentErrors.expiry}</p>}
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-gray-400 mb-1.5 uppercase tracking-wide">CVV <span className="text-amber-400">*</span></label>
                        <div className="relative">
                          <input
                            name="cvv"
                        ...
                        {paymentErrors.cvv && <p className="text-red-400 text-xs mt-1">{paymentErrors.cvv}</p>}
                      </div>
                    </div>

                    <label className="flex items-center gap-3 cursor-pointer group">
                      <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition ${paymentForm.saveCard ? "bg-amber-400 border-amber-400" : "border-white/20 group-hover:border-amber-400/50"}`}
                        onClick={() => setPaymentForm({ ...paymentForm, saveCard: !paymentForm.saveCard })}>
                        {paymentForm.saveCard && <i className="fa-solid fa-check text-gray-900 text-xs"></i>}
                      </div>
                      <span className="text-gray-400 text-sm group-hover:text-gray-300 transition">Save card for future orders</span>
                    </label>
                  </div>
                )}
                ...
                {paymentForm.method === "cash" && (
                  <div className="bg-green-400/5 border border-green-400/20 rounded-2xl p-6 text-center">
                    ...
                    <p className="text-amber-50 font-bold mb-1">Pay when you receive your order</p>
                    <p className="text-gray-400 text-sm">Please have the exact amount ready: <span className="text-amber-400 font-black">${total.toFixed(2)}</span></p>
                  </div>
                )}
                ...
                {paymentForm.method === "wallet" && (
                  <div className="space-y-3">
                    {[
                      { icon: "fa-brands fa-paypal", name: "PayPal", color: "text-blue-400" },
                      { icon: "fa-brands fa-google-pay", name: "Google Pay", color: "text-white" },
                      { icon: "fa-brands fa-apple-pay", name: "Apple Pay", color: "text-white" },
                    ].map(({ icon, name, color }) => (
                      <div key={name} className="flex items-center gap-4 bg-gray-700 border border-white/10 hover:border-amber-400/30 rounded-xl px-5 py-4 cursor-pointer transition">
                        <i className={`${icon} text-2xl ${color}`}></i>
                        <span className="text-amber-50 font-semibold">{name}</span>
              ...
              <div className="space-y-5">
                ...
                <div className="bg-gray-800 border border-white/5 rounded-3xl p-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button onClick={() => setCurrentStep("delivery")} className="text-amber-400 text-xs font-bold hover:text-amber-300 cursor-pointer">Edit</button>
                  </div>
                  <div className="space-y-2 text-sm">
                    <p className="text-amber-50 font-semibold">{delivery_form.firstName} {delivery_form.lastName}</p>
                    {delivery_form.method === "delivery" ? (
                      <p className="text-gray-400">{delivery_form.address}{delivery_form.apartment ? `, ${delivery_form.apartment}` : ""}, {delivery_form.city}</p>
                    ) : (
                      <p className="text-gray-400">Store Pickup — 123 Food Street, Cairo</p>
                    )}
                    <p className="text-gray-400">{delivery_form.phone} · {delivery_form.email}</p>
                    {delivery_form.notes && <p className="text-amber-50/60 italic text-xs mt-1">"{delivery_form.notes}"</p>}
                  </div>
                </div>
                ...
                <div className="bg-gray-800 border border-white/5 rounded-3xl p-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button onClick={() => setCurrentStep("payment")} className="text-amber-400 text-xs font-bold hover:text-amber-300 cursor-pointer">Edit</button>
                  </div>
                  {paymentForm.method === "card" && (
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-amber-50 text-sm font-semibold">•••• •••• •••• {paymentForm.cardNumber.slice(-4) || "0000"}</p>
                        <p className="text-gray-400 text-xs">{paymentForm.cardName}</p>
                      </div>
                    </div>
                  )}
                  {paymentForm.method === "cash" && (
                    <div className="flex items-center gap-3">
                      ...
                      <p className="text-amber-50 text-sm font-semibold">Cash on Delivery</p>
                    </div>
                  )}
                  {paymentForm.method === "wallet" && (
                    <div className="flex items-center gap-3">
                      <i className="fa-solid fa-wallet text-amber-400 text-xl"></i>
                      <p className="text-amber-50 text-sm font-semibold">Digital Wallet</p>
                    </div>
                  )}
                </div>
                  ...
                  <ul className="space-y-3">
                    {cartItems.map((item) => (
                      <li key={item.id} className="flex items-center gap-3">
                        <img src={item.src} alt={item.name} className="w-12 h-12 object-cover rounded-xl shrink-0" />
                        <div className="flex-1 min-w-0">
                          <p className="text-amber-50 text-sm font-semibold truncate">{item.name}</p>
                          <p className="text-gray-400 text-xs">Qty: {item.quantity} × ${item.price.toFixed(2)}</p>
                        </div>
                        <p className="text-amber-400 font-bold text-sm">${(item.price * item.quantity).toFixed(2)}</p>
                      </li>
                    ))}
                  </ul>
                </div>
                ...
                <div className="flex gap-3">
                  <button onClick={() => setCurrentStep("payment")} className="flex-1 border-2 border-white/10 hover:border-white/20 text-amber-50 font-bold py-3.5 rounded-xl transition cursor-pointer flex items-center justify-center gap-2">
                    <i className="fa-solid fa-arrow-left text-sm"></i>
                    Back
                  </button>
                  <button
                  ...
                  >
                    {loading ? (
                      ...
                      <><i className="fa-solid fa-lock text-sm"></i>Place Order · ${total.toFixed(2)}</>
                    )}
                  </button>
                </div>
              </div>
            ...
            <div className="bg-gray-800 border border-white/5 rounded-3xl p-6 sticky top-28">
                ...
                {cartItems.map((item) => (
                  <li key={item.id} className="flex gap-3 items-center">
                    <div className="relative shrink-0">
                      <img src={item.src} alt={item.name} className="w-12 h-12 object-cover rounded-xl" />
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-amber-50 text-xs font-semibold truncate">{item.name}</p>
                      <p className="text-gray-500 text-xs">${item.price.toFixed(2)} each</p>
                    </div>
                    <p className="text-amber-50 text-sm font-bold">${(item.price * item.quantity).toFixed(2)}</p>
                  </li>
                ))}
              </ul>
              ...
              <div className="border-t border-white/10 pt-4 space-y-2.5">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className={delivery === 0 ? "text-green-400 font-semibold" : "text-amber-50"}>
                ...
                <div className="border-t border-white/10 pt-3 flex justify-between items-center">
                  <span className="text-amber-50 font-black text-lg">Total</span>
